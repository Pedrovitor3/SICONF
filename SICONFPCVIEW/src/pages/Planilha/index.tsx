...
export default function Planinha({ frequenciaId, setChave }: Props) {
  ...
  const loadingPessoa = async () => {
    ...
    try {
      ...
      if (response !== false) {
        ...
        const sortedDocuments = filteredPessoa.sort((a: any, b: any) => {
          // Assuming "position" is a numeric field, you may need to parse it to numbers
          return parseInt(a.position, 10) - parseInt(b.position, 10);
        });

        setPessoa(sortedDocuments);
      } else {
        message.error(
          'Ocorreu um erro inesperado ao obter os dados das pessoas.',
        );
      }
    } catch (error) {
      message.error(
  ...
  const clickDeletePessoa = async (id: any) => {
    ...
    if (response !== false) {
      ...
      const newPessoas = pessoa.filter(pessoa => pessoa.id !== id);

      setPessoa(await reorderPositions(newPessoas, position));
      loadingPessoa();
    }
  };
  ...
  const columns = [
    {
      title: 'N°',
      dataIndex: 'position', // Use a nova propriedade
      key: 'position',
